...
import { Bot, Plus, MessageSquare, Settings, Trash2, Pin, Search, X, Zap } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[260px] flex flex-col border-r" style={{ height: "100dvh", background: "var(--color-surface)", borderColor: "var(--color-border)" }}>
      {/* Header */}
      <div className="h-14 flex items-center px-4 gap-3 border-b flex-shrink-0" style={{ borderColor: "var(--color-border)" }}>
        <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "var(--color-cyan2)" }}>
      ...
      {/* Actions */}
      <div className="p-3 flex gap-2 border-b flex-shrink-0" style={{ borderColor: "var(--color-border)" }}>
        <button onClick={handleNew} className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium" style={{ background: "var(--color-cyan2)", color: "#000" }}>
          <Plus size={15} /> New Chat
        </button>
        <button onClick={() => setShowSearch(!showSearch)} className="p-2 rounded-lg" style={{ background: showSearch ? "var(--color-surface3)" : "var(--color-surface2)", color: showSearch ? "var(--color-cyan)" : "var(--color-muted)" }}>
          <Search size={16} />
        </button>
      </div>
          ...
          <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden flex-shrink-0">
            <div className="p-3 pt-2">
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: "var(--color-surface2)", border: "1px solid var(--color-border)" }}>
                ...
                <input autoFocus value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="flex-1 bg-transparent text-sm outline-none" style={{ color: "var(--color-text)" }} />
      ...
      <div className="flex-1 overflow-y-auto p-2 min-h-0">
        {pinned.length > 0 && (
          <div className="mb-2">
            ...
            {pinned.map((c) => (
              <ConvItem key={c.id} conv={c} active={activeConversationId === c.id} onPin={handlePin} onDelete={handleDelete}
                onClick={() => { setActiveConversationId(c.id); navigate(`/conversations/${c.id}`); }} />
            ))}
          </div>
        )}
        {recent.length > 0 && (
      ...
      <div className="p-3 border-t flex-shrink-0" style={{ borderColor: "var(--color-border)" }}>
        <button
          ...
          style={{ color: "var(--color-muted)", background: "var(--color-surface2)", border: "1px solid var(--color-border)" }}
          onMouseEnter={(e) => { e.currentTarget.style.color = "white"; e.currentTarget.style.borderColor = "var(--color-cyan2)"; }}
...
function ConvItem({ conv, active, onClick, onPin, onDelete }: any) {
  ...
  return (
    <motion.div
      ...
      style={{
        background: active ? "var(--color-surface3)" : hover ? "var(--color-surface2)" : "transparent",
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs" style={{ color: "var(--color-muted)" }}>{conv.message_count || 0} msgs</div>
      </div>
      {(hover || active) && (
        <div className="flex items-center gap-0.5">
          <button onClick={(e) => onPin(conv.id, conv.pinned, e)} className="p-1 rounded" style={{ color: conv.pinned ? "var(--color-cyan)" : "var(--color-muted)" }}><Pin size={11} /></button>
          <button onClick={(e) => onDelete(conv.id, e)} className="p-1 rounded" style={{ color: "var(--color-red)" }}><Trash2 size={11} /></button>
        </div>
      )}
    </motion.div>
  );
